...
class TylerAgentExecutor(AgentExecutor):
    ...
    Supports real-time token streaming when streaming is enabled,
    emitting TaskArtifactUpdateEvent for each token chunk.
    """
    
    def __init__(self, agent, streaming: bool = True):
        """Initialize the executor.
        
        Args:
            agent: The Tyler Agent instance to wrap
            streaming: Whether to stream tokens in real-time (default: True)
        """
        self.agent = agent
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        We just need to:
        - Run the Tyler agent (with or without streaming)
        - Emit status updates and artifacts via the event queue
        
        When streaming is enabled, emits TaskArtifactUpdateEvent for each
        token chunk as it's generated by the LLM.
        ...
        try:
            ...
            if self._streaming_enabled:
                # Streaming execution: emit chunks as they arrive
                await self._execute_streaming(
                    task_id, context_id, tyler_thread, event_queue, execution
                )
            else:
                # Non-streaming execution: wait for complete response
    ...
    async def _execute_streaming(
        ...
        """Execute with streaming, emitting chunks as they arrive.
        
        Args:
            task_id: The A2A task ID
            context_id: Optional context ID
            tyler_thread: The Tyler thread to process
            event_queue: Queue for sending events
            execution: The execution tracking object
        """
        ...
        async for event in self.agent.stream(tyler_thread):
            if event.type == EventType.LLM_STREAM_CHUNK:
                ...
                logger.error(f"Streaming error for task {task_id}: {error_msg}")
                ...
                # Streaming complete - send final events
                logger.debug(f"Streaming complete for task {task_id}: {chunk_count} chunks")
                break
        ...
        logger.info(f"Completed streaming task {task_id}: {chunk_count} chunks, {total_bytes} bytes")
    ...
    async def _execute_non_streaming(
        ...
        """Execute without streaming, waiting for complete response.
        ...
        Args:
            ...
            execution: The execution tracking object
        """
        # Execute Tyler agent (non-streaming)
...
class A2AServer:
    ...
    Supports real-time token streaming when the streaming parameter is enabled,
    allowing A2A clients to receive response tokens as they're generated.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            streaming: Enable real-time token streaming (default: True).
                      When enabled, clients receive response tokens as they're
                      generated via TaskArtifactUpdateEvent with append=True.
        """
        if not HAS_A2A:
            raise ImportError(
        ...
        # Create executor with streaming configuration
        self._executor = TylerAgentExecutor(agent, streaming=streaming)
        ...
        logger.info(f"A2A server initialized with streaming={'enabled' if streaming else 'disabled'}")

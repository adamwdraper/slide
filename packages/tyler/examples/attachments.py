...
async def setup():
    ...
    # Create thread store using factory pattern
    thread_store = await ThreadStore.create()
    
    # Create file store using factory pattern
    ...
    return thread_store, agent, file_store

async def example_manual_attachment(thread_store, file_store):
...
async def example_tool_generated_attachment(thread_store, agent, file_store):
...
async def main():
    # Run examples
    try:
        thread_store, agent, file_store = await setup()
        thread1 = await example_manual_attachment(thread_store, file_store)
        thread2 = await example_tool_generated_attachment(thread_store, agent, file_store)
        thread3 = await example_adding_attachment_to_existing_message(thread_store, file_store)

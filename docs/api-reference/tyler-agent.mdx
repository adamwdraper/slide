...
`result.success` is `True` when no `EXECUTION_ERROR` event was emitted. When using `response_type`, the `structured_data` field contains the validated Pydantic model instance.

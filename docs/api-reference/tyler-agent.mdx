...
<Warning>
  Raw mode is for advanced use cases requiring OpenAI compatibility. It does NOT execute tools or handle iterations.
</Warning>
...
**Limitations of raw mode:**
- Tools are NOT executed (tool call deltas are just passed through)
- No multi-turn iteration support
- No ExecutionEvent telemetry
- Raw chunks must be handled by consumer (SSE serialization, etc.)

...
description: 'Build agents that remember conversations and maintain context'
---

Conversation memory is crucial for building agents that feel intelligent and contextual. With Slide's Narrator integration, your agents can remember past interactions, maintain conversation history, and provide personalized experiences.
...
With memory, your agent becomes truly useful:
...
async def create_agent_with_memory():
    ...
    agent = Agent(
        name="assistant",
        model_name="gpt-4",
        purpose="To be a helpful assistant that remembers our conversations",
...
# Use the agent
async def main():
    agent, thread_store, _ = await create_agent_with_memory()
...
response = await bot.handle_ticket(
    ticket_id="12345",
...
<CardGroup cols={2}>
  ...
  >
    Test agents with persistent memory
  </Card>
  <Card

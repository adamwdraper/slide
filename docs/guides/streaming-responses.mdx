...
### Thinking in Message Metrics

After streaming completes, thinking content is stored in the message's `metrics` dict:
...
async for event in agent.go(thread, stream=True):
    if event.type == EventType.MESSAGE_CREATED:
        ...
        if msg.role == "assistant":
            ...
            if 'reasoning_content' in msg.metrics:
                reasoning = msg.metrics['reasoning_content']
                print(f"Full reasoning: {reasoning}")
            
            # Anthropic models may also include structured thinking_blocks
            if 'thinking_blocks' in msg.metrics:
                blocks = msg.metrics['thinking_blocks']
                print(f"Thinking blocks: {len(blocks)}")
```

...
<CardGroup cols={2}>
  <Card
    title="Agent Delegation"
    icon="users"
    href="https://github.com/adamwdraper/slide/blob/main/packages/tyler/examples/400_agent_delegation.py"
  >
...
result = await agents["researcher"].run(thread)
```
...
class DynamicCoordinator:
    ...
    async def handle_task(self, task: str) -> Any:
        ...
        return await coordinator.run(thread)
```
...
result = await synthesizer.run(thread)
```
...
async def parallel_research(topics: List[str]):
    ...
    for topic in topics:
        ...
        research_tasks.append(agent.run(thread))
...
class ResilientCoordinator:
    ...
    async def delegate_with_fallback(self, task: str):
        ...
        thread = Thread()
        thread.add_message(Message(role="user", content=task))
        return await self.fallback.run(thread)
```

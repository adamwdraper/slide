...
<CardGroup cols={2}>
  <Card
    ...
    icon="database"
    href="https://github.com/adamwdraper/slide/blob/main/packages/tyler/examples/005_thread_persistence.py"
  >
  ...
  <Card
    title="Attachments"
    icon="paperclip"
    href="https://github.com/adamwdraper/slide/blob/main/packages/tyler/examples/200_attachments.py"
  >
...
class ConversationSession:
    ...
    async def send_message(self, content: str):
        ...
        result = await self.agent.run(self.thread)
...
async def summarize_old_conversations(thread: Thread, max_messages: int = 50):
    if len(thread.messages) > max_messages:
        ...
        summary_result = await summary_agent.run(summary_thread)

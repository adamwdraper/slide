...
Instead of defining agent settings in code, you can use YAML configuration files. This is especially useful when you want to share the same configuration between the CLI and your Python code.
...
<Steps>
  ...
  <Step title="Load Agent from Config">
    ...
    async def main():
        ...
        # Or use auto-discovery (searches ./tyler-chat-config.yaml, etc.)
  ...
  <Step title="Environment Variables in Config">
...
<Tip>
  Configuration files are great for:
  - Sharing settings between CLI (`tyler-chat`) and Python code
  - Environment-specific configurations (dev, staging, prod)
  - Team collaboration (commit configs to version control)
  - Quick experimentation without changing code
</Tip>

See the complete example at `examples/003_agent_from_config.py` in the Tyler package.
...
<AccordionGroup>
  ...
  <Accordion title="Processing Complex Files">
    ...
    - **Unusual file formats**: Install `libmagic` for better detection
    
    These are optional - your agent will work fine without them for most use cases.
  </Accordion>
</AccordionGroup> 
...
Tyler's A2A server supports real-time token streaming. When clients call `message/stream`, they receive response tokens as they're generated by the LLM via Server-Sent Events (SSE).

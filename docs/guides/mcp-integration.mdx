...
thread = Thread()
thread.add_message(Message(role="user", content="Search W&B docs for Weave"))
...
```python
{
    ...
    "transport": str,   # Required: "stdio", "sse", or "websocket"
    ...
    # Optional fields:
    "headers": Dict,           # Custom HTTP headers (for sse/websocket)
...
**WebSocket** - For WebSocket MCP servers:
```python
{
    "name": "api",
    "transport": "websocket",

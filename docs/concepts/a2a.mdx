...
A2A clients can receive response tokens as they're generated via Server-Sent Events (SSE):
...
# Create a task
task_id = await client.create_task("agent", "Write a detailed analysis")

# Stream response tokens in real-time
async for message in client.stream_task_messages("agent", task_id):
    # Tokens arrive as they're generated by the LLM
...
- **`message/send`**: Returns aggregated response after completion
- **`message/stream`**: Streams tokens via SSE as they're generated
...
# Send task and wait for complete response (non-streaming)
result = await client.send_task("agent", "Summarize this document")

# Result contains the full response and artifacts

...
<Note>
**Updated API**: Tyler now supports MCP through declarative configuration with `Agent(mcp={...})`.
See the [MCP Integration Guide](/guides/mcp-integration) for the complete, up-to-date documentation.
</Note>
...
# Create agent with MCP config (validates schema)
agent = Agent(
    name="Tyler",
    model_name="gpt-4.1",
    tools=["web"],
...
async def execute_with_fallback(agent: Agent, task: str):
    ...
    try:
        ...
        if agent.mcp_adapter:
            ...
            return await agent.run(fallback_thread)
        raise
```

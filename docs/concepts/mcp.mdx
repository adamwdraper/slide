...
async def discover_and_use_tools(adapter: MCPAdapter, task: str):
    ...
    agent = Agent(
        name="dynamic-agent",
        mcp_adapter=adapter,
        system_prompt=f"You have access to these tools: {relevant_tools}"
    )
...
agent = Agent(
    name="hybrid-agent",
    tools=[web.search, files.read],  # Native Slide tools
    ...
    try:
        ...
        if agent.mcp_adapter:
            return await agent.run(
                task, 
                tool_choice="mcp"  # Force MCP tool usage
            )
        raise
```
...
result = await agent.run(
    "Analyze this image and create a summary report"
)
```
